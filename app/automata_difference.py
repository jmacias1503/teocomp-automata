...
def difference_automata(json1, json2):
    alphabet = list(set(json1["alphabet"]) | set(json2["alphabet"])) #set para eliminar duplicados
    states = list(product(json1["states"], json2["states"]))
    initial_state = (json1["initial_state"], json2["initial_state"])
    final_states = [
    state for state in states 
    if state[0] in json1["final_states"] and state[1] not in json2["final_states"]
    ] # Not in funciona como la resta de la diferencia de conjuntos
...
def print_quintuple(automata, name):
    ...
    Args:
        automata (dict): Diccionario con la estructura del AFD.
...
# Generar el autómata diferencia
automata_difference = difference_automata(json1, json2)

# Mostrar la quintupla del autómata resultante
print_quintuple(automata_difference, "Diferencia (L1 - L2)")

# Dibujar el autómata resultante
draw_automaton(automata_difference, 'finite_automaton_difference.gv')