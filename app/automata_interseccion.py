...
def intersect_automata(json1, json2):
    # Validar que los alfabetos sean compatibles
    common_alphabet = list(set(json1["alphabet"]) & set(json2["alphabet"]))
    if not common_alphabet:
        raise ValueError("Los alfabetos no tienen símbolos en común.")
    
    # Generar estados usando product (igual que en union)
    states = list(product(json1["states"], json2["states"]))
    initial_state = (json1["initial_state"], json2["initial_state"])
    
    # La diferencia clave: solo son finales si AMBOS estados componentes son finales
    final_states = [
        state for state in states
        if state[0] in json1["final_states"] and state[1] in json2["final_states"]
    ]
...
# Generar el autómata de intersección
automata_intersection = intersect_automata(json1, json2)

# Mostrar la quintupla del autómata resultante
print_quintuple(automata_intersection, "Intersección")

# Dibujar el autómata resultante
draw_automaton(automata_intersection, 'finite_automaton_intersection.gv')